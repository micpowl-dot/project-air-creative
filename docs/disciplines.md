# 🎯 Team Disciplines & Capabilities

> **Purpose:** Fill in your name, strengths, tools, and capacity so we can resource deliverables.

## How to Use This Doc

1. Find your discipline (or add one)
2. Add yourself to the roster table
3. Be honest about proficiency
4. Update availability as it changes

---

## Proficiency Key

| Level | Meaning |
|-------|---------|
| **Lead** | Can own end-to-end |
| **Strong** | Can execute independently |
| **Support** | Can assist or prep files |
| **Learning** | Developing — pair with a Lead |

---

### 1. Brand Identity & Graphic Design

| Name | Proficiency | Tools | Availability (hrs/wk) | Notes |
|------|-------------|-------|-----------------------|-------|
| Michael Powell | Lead | Adobe Creative Suite | TBD | Creative Director, AD |
| Lee Payne | Strong | Adobe Creative Suite | TBD | Senior Graphic Designer |
| Camille Nettles | Strong | Adobe Creative Suite | TBD | Associate Creative Director |

### 2. Motion Design & Animation

| Name | Proficiency | Tools | Availability (hrs/wk) | Notes |
|------|-------------|-------|-----------------------|-------|
| Michael Powell | Lead | After Effects, Adobe Creative Suite | TBD | Creative Director, AD |

### 3. Copywriting & Content Strategy

| Name | Proficiency | Tools | Availability (hrs/wk) | Notes |
|------|-------------|-------|-----------------------|-------|
| Mark Fredo | Lead | TBD | TBD | Executive Creative Director |
| Jeff Hampton | Lead | Google Docs, Gemini | TBD | Associate Creative Director |

### 4. Presentation Design

| Name | Proficiency | Tools | Availability (hrs/wk) | Notes |
|------|-------------|-------|-----------------------|-------|
| | | | | |

### 5. Print Production & Physical Fabrication

| Name | Proficiency | Tools | Availability (hrs/wk) | Notes |
|------|-------------|-------|-----------------------|-------|
| | | | | |

### 6. Swag & Merchandise

| Name | Proficiency | Tools | Availability (hrs/wk) | Notes |
|------|-------------|-------|-----------------------|-------|
| | | | | |

### 7. Event Design & Environmental Graphics

| Name | Proficiency | Tools | Availability (hrs/wk) | Notes |
|------|-------------|-------|-----------------------|-------|
| | | | | |

### 8. AI-Assisted Creative

| Name | Proficiency | Tools | Availability (hrs/wk) | Notes |
|------|-------------|-------|-----------------------|-------|
| Jeff Hampton | Lead | ElevenLabs, Google Flow, Gemini, Nanobanana | TBD | ACD — Primary AI resource |
| Lee Payne | Support | Adobe Firefly / AI tools | TBD | Senior Graphic Designer |

### 9. Video & AV Production

| Name | Proficiency | Tools | Availability (hrs/wk) | Notes |
|------|-------------|-------|-----------------------|-------|
| | | | | |

### 10. Project Management & Coordination

| Name | Proficiency | Tools | Availability (hrs/wk) | Notes |
|------|-------------|-------|-----------------------|-------|
| Denise Denson | Lead | TBD | TBD | Traffic Manager — schedules work, moves files, coordinates vendors |

---

## Deliverable ↔ Discipline Map

| Deliverable | Lead Discipline | Supporting | Assigned Lead | Gap? |
|-------------|----------------|------------|---------------|------|
| D1 — Posters | Brand Identity | AI Creative, Print, Copy | — | — |
| D2 — Badges | Brand Identity | Print, Event Design | — | — |
| D3 — Logo | Brand Identity | Motion, AI Creative | — | — |
| D4 — PPT | Presentation Design | Brand Identity, Copy | — | — |
| D5 — Swag | Swag & Merch | Brand Identity, Print, PM | — | — |

---

*Last updated: 2026-05-05*
